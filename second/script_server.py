...
from second.utils import model_tool
...
def train_nuscenes_pp():
    config = Path(__file__).resolve().parent / "configs/nuscenes/pp.nu.config"
...
def train_nuscenes_all():
    config = Path(
        __file__).resolve().parent / "configs/nuscenes/all-s.fhd.proto"
    ckpt_path = "/home/yy/deeplearning/voxelnet_torch_sparse/car_fhd_small_v1/voxelnet-27855.tckpt"
    # config = Path(__file__).resolve().parent() / "configs/car.fhd.nu.config"
    config = _get_config(config)
    _nuscenes_modify_step(config, 50, 5, 8)
    model_dir_root = Path("/home/yy/deeplearning/model_dirs/nuscene")
    date_str = datetime.datetime.now().strftime("%y%m%d_%H%M%S")
    train(config, model_dir_root / "all_fhd" / ("test_" + date_str))


def train_nuscenes_pp_all():
    config = Path(__file__).resolve().parent / "configs/nuscenes/all.pp.config"
    ckpt_path = "/home/yy/deeplearning/model_dirs/kitti/car_pp_long_v0/voxelnet-296960.tckpt"
    ckpt_path = "/home/yy/deeplearning/model_dirs/kitti/car_pp_long_v0/voxelnet-296960.tckpt"
    # config = Path(__file__).resolve().parent() / "configs/car.fhd.nu.config"
    config = _get_config(config)
...
def train_nuscenes_pp_all_sample_v2():
    ...
    model_dir_root = Path("/home/yy/deeplearning/model_dirs/nuscene")
    date_str = datetime.datetime.now().strftime("%y%m%d_%H%M%S")
    train(config, model_dir_root / "all_pp_sample_v2" / ("test_" + date_str))
...
def train_nuscenes_pp_all_v2():
    ...
    model_dir_root = Path("/home/yy/deeplearning/model_dirs/nuscene")
    date_str = datetime.datetime.now().strftime("%y%m%d_%H%M%S")
    train(config, model_dir_root / "all_pp_v2" / ("test_" + date_str))
...
def train_nuscenes_pp_vel():
    ...
    model_dir_root = Path("/home/yy/deeplearning/model_dirs/nuscene")
    date_str = datetime.datetime.now().strftime("%y%m%d_%H%M%S")
    train(config, model_dir_root / "pp_vel" / ("test_" + date_str))
...
def train_nuscenes_pp_car():
    config = Path(__file__).resolve().parent / "configs/nuscenes/car.pp.config"
    ckpt_path = "/home/yy/deeplearning/model_dirs/kitti/car_pp_long_v0/voxelnet-296960.tckpt"
    ckpt_path = "/home/yy/deeplearning/model_dirs/kitti/car_pp_long_v0/voxelnet-296960.tckpt"
    # config = Path(__file__).resolve().parent() / "configs/car.fhd.nu.config"
    config = _get_config(config)
    ...
    model_dir_root = Path("/home/yy/deeplearning/model_dirs/nuscene")
    date_str = datetime.datetime.now().strftime("%y%m%d_%H%M%S")
    train(config, model_dir_root / "pp_car" / ("test_" + date_str))
...
def resume_nuscenes_pp_all():
    ...
    model_dir_root = Path("/home/yy/deeplearning/model_dirs/nuscene")
    train(config,
          model_dir_root / "all_pp" / ("test_190424_232942"),
          resume=True)
