...
def create_groundtruth_database(dataset_class_name,
    ...
    if database_save_path is None:
        database_save_path = root_path / db_dirname
    else:
        database_save_path = Path(database_save_path)
    db_info_save_path = Path(str(info_path).replace('infos', 'dbinfos'))
    ...
    print('dbinfo saved: ', db_info_save_path)
...
def create_groundtruth_database_parallel(dataset_class_name,
    ...
    # group_counter = 0
    for image_idx in prog_bar(list(range(len(dataset)))):
        sensor_data = dataset.get_sensor_data(image_idx)
        if "image_idx" in sensor_data["metadata"]:
            image_idx = sensor_data["metadata"]["image_idx"]
        points = sensor_data["lidar"]["points"]
        annos = sensor_data["lidar"]["annotations"]
