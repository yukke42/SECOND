...
class NuScenesDataset(Dataset):
    ...
    def __init__(self,
        ...
        with open(info_path, 'rb') as f:
            data = pickle.load(f)
        self._nusc_infos = list(sorted(data["infos"], key=lambda e: e["timestamp"]))
    ...
    def ground_truth_annotations(self):
        ...
        for info in self._nusc_infos:
            ...
            mask = np.isin(gt_names, self._kitti_name_mapping)
            gt_names = gt_names[mask]
            gt_boxes = gt_boxes[mask]
            num_lidar_pts = num_lidar_pts[mask]

            gt_names_mapped = [self._kitti_name_mapping[n] for n in gt_names]
            det_range = np.array([self._eval_cfg.class_range[n] for n in gt_names_mapped])
    ...
    def get_sensor_data(self, query):
        ...
        use_second_fmt_dir = False
        if isinstance(query, dict):
            assert "lidar" in query
            idx = query["lidar"]["idx"]
            use_second_fmt_dir = query["lidar"].get('use_second_fmt_dir', False)
        ...
        if self._metadata['flat_vehicle_coords']:
        ...
        points = np.zeros((pointcloud.points.shape[1], 5), dtype=np.float32)
        points[:, :4] = pointcloud.points.T
        points[:, 3] /= 255
        points[:, 4] = 0
        sweep_points_list = [points]
        ts = info["timestamp"] / 1e6

        # Note: you do not need to sort sweeps by timestamp,
        #       because frames are added from prev one by one
        ...
        if 'gt_boxes' in info:
            ...
            if not self._metadata['second_format_direction'] and \
                    use_second_fmt_dir:
...
def _fill_trainval_infos(nusc,
    ...
    for sample in prog_bar(nusc.sample):
        ...
        if use_flat_vehicle_coords:
            # FIXME: this transformation has bug
            # global->flat-ego'
            l2e_r_mat = np.eye(3)
            l2e_t = np.zeros(3)
            e2g_yaw = Quaternion(e2g_r).yaw_pitch_roll[0]
            e2g_r_qua = Quaternion(
                scalar=np.cos(e2g_yaw / 2),
                vector=[0, 0, np.sin(e2g_yaw / 2)]
            )
            e2g_r_mat = e2g_r_qua.rotation_matrix

            info["lidar2ego_rotation"] = Quaternion(matrix=l2e_r_mat).elements.tolist()
            info["lidar2ego_translation"] = l2e_t.tolist()
            info["ego2global_rotation"] = e2g_r_qua.elements.tolist()
        else:
            # global->ego'->lidar
        ...
        if not is_test:
            ...
            gt_names = [b.name for b in boxes]
            ...
            if use_flat_vehicle_coords:
                _, boxes_flat_vehicle, _ = nusc.get_sample_data(
                    lidar_token,
            ...
            for i in range(len(gt_names)):
                if gt_names[i] in NuScenesDataset.NameMapping:
                    gt_names[i] = NuScenesDataset.NameMapping[gt_names[i]]
...
def create_nuscenes_infos(root_path,
                          ...
                          filename_template='infos_splits_sweepsN.pkl',
                          max_sweeps=10,
    ...
    if version == "v1.0-trainval":
        ...
        filename_template = filename_template.replace('splits', 'splits_mini')
    else:
    ...
    metadata = {
        'version': version,
        'max_sweeps': max_sweeps,
        'flat_vehicle_coords': use_flat_vehicle_coords,
    ...
    if is_test:
        ...
        filename_test = filename_template.replace('splits', 'test')
        with open(root_path / filename_test, 'wb') as f:
            pickle.dump(data, f)
        print(f'Created: {root_path / filename_test}')
    else:
        ...
        filename_train = filename_template.replace('splits', 'train')
        with open(root_path / filename_train, 'wb') as f:
            pickle.dump(data, f)
        print(f'Created: {root_path / filename_train}')

        filename_val = filename_template.replace('splits', 'val')
        data["infos"] = val_nusc_infos
        with open(root_path / filename_val, 'wb') as f:
            pickle.dump(data, f)
        print(f'Created: {root_path / filename_val}')
