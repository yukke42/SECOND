...
def draw_bbox_in_ax(ax,
    ...
    for bbox, rot, e_color, label in zip(bboxes, rotations, edgecolors,
                                         labels):
        rect_p = patches.Rectangle(bbox[:2],
                                   bbox[2],
                                   bbox[3],
                                   rot,
                                   fill=False,
...
def draw_2d_bbox_in_ax(ax,
    ...
    for box, color in zip(bboxes, colors):
        ...
        if with_arrow:
            ...
            start = np.mean(np.concatenate(
                [center[np.newaxis, ...], box[behind_axes]]),
                            axis=0)
            front_axes = [i for i in range(4) if i not in behind_axes]
            end = np.mean(np.concatenate(
                [center[np.newaxis, ...], box[front_axes]]),
                          axis=0)
            ax.arrow(start[0],
                     start[1],
                     end[0] - start[0],
...
def draw_3d_bbox_in_3dax(ax, bboxes, colors='r', alpha=0.25, facecolors=None):
    ...
    for box, color, facecolor in zip(bboxes, colors, facecolors):
        ...
        mp3dcoll = Poly3DCollection(verts,
                                    linewidths=1,
                                    edgecolors=color,
                                    alpha=alpha,
...
def draw_3d_bboxlines_in_pyqt(widget,
    ...
    if line_item is None:
        line_item = gl.GLLinePlotItem(pos=total_lines,
                                      color=total_colors,
                                      width=width,
                                      antialias=True,
                                      mode='lines')
        ...
        line_item.setData(pos=total_lines,
                          color=total_colors,
                          width=width,
                          antialias=True,
                          mode='lines')
    ...
    if labels is not None:
        if label_item is None:
            ...
            label_item.setData(pos=bboxes[:, 0, :],
                               text=labels,
