...
def calculate_iou_partly(gt_annos,
    ...
    for num_part in split_parts:
        ...
        if metric == 0:
            ...
            dt_boxes = np.concatenate([loc, dims, rots[..., np.newaxis]],
                                      axis=1)
            overlap_part = box3d_overlap(gt_boxes,
                                         dt_boxes,
                                         z_axis=z_axis,
...
def do_coco_style_eval(gt_annos,
    ...
    mAP_bbox, mAP_bev, mAP_3d, mAP_aos = do_eval_v2(gt_annos,
                                                    dt_annos,
                                                    current_classes,
                                                    min_overlaps,
                                                    compute_aos,
...
def get_official_eval_result(gt_annos,
    ...
    for j, curcls in enumerate(current_classes):
        ...
        for i in range(min_overlaps.shape[0]):
            ...
            detail[class_name][
                f"bbox@{min_overlaps[i, 0, j]:.2f}"] = mAPbbox.tolist()
            detail[class_name][
                f"bev@{min_overlaps[i, 1, j]:.2f}"] = mAPbev.tolist()
            detail[class_name][
                f"3d@{min_overlaps[i, 2, j]:.2f}"] = mAP3d.tolist()
    ...
    for anno in dt_annos:
        if anno['alpha'].shape[0] != 0:
            if anno['alpha'][0] != -10:
                compute_aos = True
            break
    mAPbbox, mAPbev, mAP3d, mAPaos = do_coco_style_eval(gt_annos,
                                                        dt_annos,
                                                        current_classes,
                                                        overlap_ranges,
                                                        compute_aos,
