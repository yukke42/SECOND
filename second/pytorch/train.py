    ...
    for k, v in example.items():
        if k in float_names:
            ...
            for k1, v1 in v.items():
                calib[k1] = torch.tensor(v1, dtype=dtype,
                                         device=device).to(dtype)
...
def filter_param_dict(state_dict: dict,
                      include: str = None,
                      exclude: str = None):
...
def train(config_path,
    ...
    if num_workers is not None:
        ...
        input_cfg.preprocess.num_workers = num_workers
        eval_input_cfg.preprocess.num_workers = num_workers

    input_cfg.batch_size = input_cfg.batch_size * num_gpu
    input_cfg.preprocess.num_workers = input_cfg.preprocess.num_workers * num_gpu
    ...
    if data_root and osp.exists(data_root):
        ...
        input_cfg.dataset.kitti_info_path = osp.join(data_root, train_info_filename)
        eval_info_filename = osp.basename(eval_input_cfg.dataset.kitti_info_path)
        eval_input_cfg.dataset.kitti_root_path = data_root
        eval_input_cfg.dataset.kitti_info_path = osp.join(data_root, eval_info_filename)
        if input_cfg.preprocess.database_sampler.database_info_path:
            db_info_filename = osp.basename(input_cfg.preprocess.database_sampler.database_info_path)
            input_cfg.preprocess.database_sampler.database_info_path = osp.join(data_root, db_info_filename)
    ...
    # FIXME: 'steps' is misleading (refactor together later)
    train_epochs = train_cfg.steps
    train_epochs_per_eval = train_cfg.steps_per_eval
    train_cfg.steps = train_cfg.steps * len(dataloader)
    train_cfg.steps_per_eval = train_cfg.steps_per_eval * len(dataloader)
    ...
    model_logging.log_text(f'Total Batch Size {input_cfg.batch_size} with {num_gpu} GPUs', 0, tag="config")
    model_logging.log_text(f'train {train_epochs} epochs and '
                           f'evaluate at every {train_epochs_per_eval} epochs', 0, tag="config")
    model_logging.log_text(f'train {total_step} steps and '
                           f'evaluate at every {steps_per_eval} steps\n', 0, tag="config")
...
def mcnms_parameters_search(config_path, model_dir, preds_path):
    pass
