...
from second.pytorch.core.losses import (WeightedSmoothL1LocalizationLoss,
...
class VoxelNet(nn.Module):
    ...
    def loss(self, example, preds_dict):
        ...
        self.start_timer("forward.loss.prep_weight")
        ...
        self.end_timer("forward.loss.prep_weight")

        self.start_timer("forward.loss.compute")
        ...
        if self._use_direction_classifier:
            ...
            dir_loss = self._dir_loss_ftor(dir_logits,
                                           dir_targets,
                                           weights=weights)
            dir_loss = dir_loss.sum() / batch_size_dev
            loss += dir_loss * self._direction_loss_weight
        self.end_timer("forward.loss.compute")
    ...
    def forward(self, example):
        ...
        self.end_timer('forward.prep')

        # features: [num_voxels, max_num_points_per_voxel, 7]
        # num_points: [num_voxels]
        # coors: [num_voxels, 4]
        self.start_timer('forward.network')
        ...
        self.end_timer('forward.network')

        if self.training:
            self.start_timer('forward.loss')
            loss = self.loss(example, preds_dict)
            self.end_timer('forward.loss')
            return loss
        else:
