...
class PillarFeatureNetRadius(nn.Module):
    ...
    def forward(self, features, num_voxels, coors):
        ...
        features_radius = torch.norm(features[:, :, :2],
                                     p=2,
                                     dim=2,
                                     keepdim=True)
        features_radius = torch.cat([features_radius, features[:, :, 2:]],
                                    dim=2)
...
class SingleVEN(nn.Module):
    ...
    def forward(self, features, num_voxels, coors):
        ...
        points_mean = features[:, :, :3].sum(
            dim=1, keepdim=True) / num_voxels.type_as(features).view(-1, 1, 1)
        features_offset_mean = features[:, :, :3] - points_mean

        # Find distance of x, y, and z from pillar center
        dtype = features.dtype
        features_offset_center = torch.zeros_like(features[:, :, :3])
        features_offset_center[:, :, 0] = features[:, :, 0] - (
                coors[:, 3].to(dtype).unsqueeze(1) * self.vx + self.x_offset)
        features_offset_center[:, :, 1] = features[:, :, 1] - (
                coors[:, 2].to(dtype).unsqueeze(1) * self.vy + self.y_offset)
        features_offset_center[:, :, 2] = features[:, :, 2] - (
                coors[:, 1].to(dtype).unsqueeze(1) * self.vz + self.z_offset)

        # combine features
        features = torch.cat([features, features_offset_mean, features_offset_center], dim=-1)
