...
def second_box_decode(box_encodings,
    ...
    if box_ndim > 7:
        xa, ya, za, wa, la, ha, ra, *cas = torch.split(anchors, 1, dim=-1)
        if encode_angle_to_vector:
            xt, yt, zt, wt, lt, ht, rtx, rty, *cts = torch.split(box_encodings,
                                                                 1,
                                                                 dim=-1)
        else:
            xt, yt, zt, wt, lt, ht, rt, *cts = torch.split(box_encodings,
                                                           1,
                                                           dim=-1)
    else:
        xa, ya, za, wa, la, ha, ra = torch.split(anchors, 1, dim=-1)
...
def multiclass_nms(nms_func,
    ...
    boxes_ids = (range(num_classes) if boxes.shape[1] > 1 else [0] *
                 num_classes)
...
def nms_v2(bboxes,
    ...
    if post_max_size is None:
        post_max_size = -1
    res = spconv.ops.nms(bboxes.cpu(), scores.cpu(), pre_max_size,
                         post_max_size, iou_threshold, 1.0)
