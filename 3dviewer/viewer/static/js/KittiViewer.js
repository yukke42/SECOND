        ...
        if (this.imageIndexes.length !== 0 && this.imageIndexes.includes(image_idx)) {
            ...
            let ajax1 = $.ajax({
                ...
                success: function (response) {
                    ...
                    if ("locs" in response){
                        let locs = response["locs"];
                        let dims = response["dims"];
                        let rots = response["rots"];
                        let labels = response["labels"];
                        self.gtBboxes = response["bbox"];
                        self.gtBoxes = boxEdgeWithLabel(
                            dims, locs, rots, 2,
                            self.gtBoxColor, labels,
                            self.gtLabelColor);
                        for (let i = 0; i < self.gtBoxes.length; ++i) {
                            scene.add(self.gtBoxes[i]);
                        }

                        // self.gtArrow = boxArrow(
                        //     dims, locs. rots,
                        //     2, "#ff0000"
                        // );
                        // console.log(self.gtArrow);
                        // for (let i = 0; i < self.gtArrow.length; i++) {
                        //     scene.add(self.gtArrow[i]);
                        // }
                    }

                    // draw detected objects
                    // if (self.drawDet && response.hasOwnProperty("dt_dims")) {
                    if (response.hasOwnProperty("dt_bbox")) {
                        let locs = response["dt_locs"];
                        let dims = response["dt_dims"];
                        let rots = response["dt_rots"];
                        let scores = response["dt_scores"];
                        self.dtBboxes = response["dt_bbox"];
                    ...
                    for (let i = 0; i < Math.min(points.length / numFeatures, self.maxPoints); i++) {
                        for (let j = 0; j < numFeatures; ++j){
                            self.pointCloud.geometry.attributes.position.array[i * 3 + j] = points[i * numFeatures + j];
                        }
                    }
                    if (self.enableInt16){
