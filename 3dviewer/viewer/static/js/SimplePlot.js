...
function boxEdgeWithLabel(dims, locs, rots, edgewidth, color, labels, lcolor) {
    ...
    for (let i = 0; i < dims.length; ++i) {
        let cube = new THREE.BoxGeometry(dims[i][0], dims[i][1], dims[i][2]);
        let edgeGeo = new THREE.EdgesGeometry(cube);
        let material = new THREE.LineBasicMaterial({
            color: color,
            linewidth: edgewidth
        });
        ...
        let labelObj = new THREE.CSS2DObject(labelDiv);
...
function boxArrow(dims, locs, rots, edgewidth, color) {
    let arrows = [];
    for (let i = 0; i < dims.length; ++i) {
        let origin = new THREE.Vector3(locs[i][0], locs[i][1], locs[i][2]);
        let direction = new THREE.Vector3(0, dims[i][1], 0);
        let length = direction.length();
        let arrowHelper = new THREE.ArrowHelper(direction.normalize(), origin, length, color);
